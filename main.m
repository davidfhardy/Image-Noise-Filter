...
Eh = edge(In,'sobel','horizontal');
Ev = edge(In,'sobel','vertical');
for i=N+1:m-N-1
    for j=N+1:n-N-1
        ...
        [Omega0,Omega1,Omega2,Omega3,Omega4,EWh,EWv]= getWindowPoints(In,Eh,Ev,i,j,N);   
        [SQMV,SQMD_B,SQMD_C] = medianVector(Omega1,Omega2,Omega3,Omega4);
        Edge = edgeDirection(EWh,EWv);
        SQMR = referenceMedian(In,i,j,Edge,SQMV,SQMD_B,SQMD_C,p);
